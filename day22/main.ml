...
module Parsing = struct
  ...
  let rotate = char 'R' *> return RotateR <|> char 'L' *> return RotateL
...
module Solving = struct
  ...
  let make_map (dimx, dimy) data =
    ...
    List.iteri data ~f:(fun y row ->
        List.iteri row ~f:(fun x c -> map.(y).(x) <- c);
  ...
  let get_start map =
    ( fst
      @@ Array.findi_exn map.(0) ~f:(fun _ c ->
             match c with Air -> true | _ -> false),
      0 )
  ...
  let add (dimx, dimy) (x, y) (x', y') = ((x + x') % dimx, (y + y') % dimy)
  ...
  let move_until map dims (pos, dir) i =
    ...
    let rec aux last_valid current n =
      if n = 0 then (last_valid, dir)
        ...
        else if is_void map next then aux last_valid next n
        else (last_valid, dir)
    in
    aux pos pos i
  ...
  let do_path map dims start path =
    let rec aux pos = function
      | [] -> pos
      | next :: todo -> aux (do_action map dims pos next) todo
    in
    aux start path

  let score =
    let score_dir = function Up -> 3 | Left -> 2 | Down -> 1 | Right -> 0 in
    fun ((x, y), dir) -> ((y + 1) * 1000) + (4 * (x + 1)) + score_dir dir
