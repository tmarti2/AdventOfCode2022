...
module Parsing = struct
  ...
  let throw =
    ...
    lift3
      (fun test yes no v -> if v mod test = 0 then yes else no)
      test restrue resfalse

  let monkey =
    lift3
      (fun worries inspect throw -> { worries; inspect; throw; activity = 0 })
      start inspect throw
...
module Solving = struct
  ...
  let round ~f monkeys = Array.iter monkeys ~f:(do_monkey ~f monkeys)
  ...
  let business monkeys =
    ...
    |> Fn.flip List.take 2 |> List.reduce_exn ~f:( * )
