...
module Solving = struct
  ...
  let next_pos arr size n =
    let (i,(_,e)) =
      Array.findi_exn arr ~f:(fun _ (i,_) -> i = n)
    in
    i, (i + e) % (size - 1), e
  ...
  let move_all arr size n =
    let rec aux n =
      if n = size then 0
        ...
        if ni < i then
          loop i (ni+1) ~f:(fun j -> arr.(j) <- arr.(j-1))
        else
          loop i ni ~f:(fun j -> arr.(j) <- arr.(j+1));
        arr.(ni) <- (n, e);
        aux (n+1)
      end
    in
  ...
  let solve input key mix =
    ...
    List.sum (module Int) [1000;2000;3000]
