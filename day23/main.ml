...
module Solving = struct
  ...
  let create_map data (_, _, mx, my) =
    ...
    let map =
      Array.make_matrix
        ~dimx:(my + uyoffset + dyoffset)
        ~dimy:(mx + lxoffset + rxoffset)
        Empty
    in
    List.iteri data ~f:(fun y row ->
        List.iteri row ~f:(fun x c -> set map c (x + lxoffset, y + uyoffset)));
  ...
  let can_move =
    ...
    fun map p i ->
      ...
      List.for_all nexts_pos ~f:(fun (x, y) ->
          match map.(y).(x) with Empty | Next _ -> true | Elf -> false)
  ...
  let rounds ?stop (map, _, elfs) =
    let rec aux i elfs n =
      if Option.is_some stop && Option.value_exn stop < n then n
        ...
        if Poly.(nelfs = elfs) then n else aux ni nelfs (n + 1)
    in
    aux 0 elfs 1
  ...
  let part2 (input : input) : output = get_data input |> rounds
end
