...
module Parsing = struct
  ...
  let to_int l = List.map l ~f:(fun c -> Char.(to_int c - to_int '0'))
...
module Solving = struct
  ...
  let is_inside x y dx dy = x >= 0 && x < dx && y >= 0 && y < dy
  ...
  let count_visibility map =
    Array.foldi ~init:0
      ~f:(fun y acc row ->
        acc + Array.counti ~f:(fun x _ -> is_visible map x y) row)
      map
  ...
  let best_scenic_score map =
    Array.foldi ~init:0
      ~f:(fun y init row ->
        Array.foldi ~init ~f:(fun x acc _ -> max acc (scenic_score map x y)) row)
      map

  let part1 (input : input) : output = count_visibility input
  let part2 (input : input) : output = best_scenic_score input
end
