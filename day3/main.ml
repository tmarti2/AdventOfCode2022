...
module Solving = struct
  ...
  let rec aux_part2 todo acc =
    ...
    | [] -> acc
    | e1 :: e2 :: e3 :: tl ->
      let value = find_common2 e1 e2 e3 |> score in
      aux_part2 tl (acc + value)
    | _ -> assert false

  let part1 (input : input) : output =
    List.sum ~f:(Fn.compose score find_common1) (module Int) input |> Option.some
