...
module Parsing = struct
  ...
  let dest1 =
    string "; tunnel leads to valve " *> word >>| fun w -> [w]
  let destn =
    string "; tunnels lead to valves " *>
    sep_by (string ", ") word

  let row = lift3 (fun name flow dest ->
      name, {flow;dest}
...
module Solving = struct
    ...
    let rec aux time_left valves_left score pos =
      if time_left < 0 then score
        ...
        List.map valves_left ~f:(fun (next, flow) ->
            ...
            let left = List.filter valves_left
                ~f:(fun (s, _) -> not (String.equal s next)) in
            aux time left score next
          )
        |> List.max_elt ~compare |> Option.value ~default:score
      )
    in
  ...
  let part2 (_input : input) : output =
    Stdlib.Format.printf "P2 TODO@.%!"; 0
end
