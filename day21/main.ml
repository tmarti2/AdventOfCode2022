...
module Parsing = struct
  ...
  let action = int <|> lift3 (fun a op b -> Op (a, op, b)) word op word
...
module Solving = struct
  ...
  let build_tree ?var monkeys =
    ...
    let rec aux name =
      ...
      | Int i, _ -> Val i
      | Op (a, op, b), _ -> Node (aux a, op, aux b)
    in
    aux "root"

  let get = function Val i -> i | _ -> assert false
  ...
  let solve equation =
    let rec aux left acc =
      match left with
      | X -> Val acc
      | Node (b, op, Val a) | Node (Val a, ((Add | Mul) as op), b) ->
          aux b (inv_op op acc a)
      | Node (Val a, ((Sub | Div) as op), b) -> aux b (get_op op a acc)
      | _ -> assert false
    in
    match equation with
    | Node (Val res, _, a) | Node (a, _, Val res) -> aux a res
    | _ -> assert false

  let part1 (input : input) : output = build_tree input |> simplify |> get
